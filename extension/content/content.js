...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    switch (message.action) {
        ...
        case "contextChanged":
            // Forward the context change notification to the sidebar
            chrome.runtime.sendMessage(
                ...
                (response) => {
                    ...
                    if (chrome.runtime.lastError) {
                        console.error(
                            "Error forwarding context change:",
        ...
        case "refreshContext":
            // Extract content and forward to sidebar
            const refreshedContent = extractPageContent();
            sendResponse(refreshedContent);
            break;
    }
...
function extractPageContent() {
    try {
        ...
        const combinedContent = formatExtractedContent(
            pageTitle,
            pageUrl,
            metaTags,
            textContent,
...
function extractVisibleText() {
    ...
    const headings = Array.from(
        bodyClone.querySelectorAll("h1, h2, h3, h4, h5, h6")
    );
